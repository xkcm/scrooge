...
type VerifierFactoryGeneratorOptions = {
  requestProperty: Extract<
    keyof ApiRequest,
    "body" | "cookies" | "params" | "query"
  >;
  errorClass: {
    new (...args: ConstructorParameters<typeof ApiError>): ApiError;
  };
};
...
const generateGenericRequestVerifierFactory =
  ...
  (options: T) =>
  (req: ApiRequest, res: ApiResponse, next: ApiNextFunc) => {
...
export const createRequestParamsVerifier =
  generateGenericRequestVerifierFactory({
    errorClass: InvalidRequestParamsError,
    requestProperty: "params",
  });

export const createRequestCookiesVerifier =
  generateGenericRequestVerifierFactory({
