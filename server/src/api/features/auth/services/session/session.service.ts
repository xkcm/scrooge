...
const sessionService: SessionService = {
  async createSession(userId, sessionInfo) {
    ...
    const expiresAt = createdAt + serverConfig.service_configs.session.expire_time;
    ...
    await sessionRedisService.saveSessionInfo(createdSession.id, "last_used", Date.now());

    return createdSession;
  },

  async getSessionsByUserId(userId) {
    const foundSessions = await prismaClient.session.findMany({
      where: { userId },
      select: {
        id: true,
        createdAt: true,
    ...
    const sessionsWithRedisInfo = await Promise.all(foundSessions.map(async (session) => ({
  ...
  getSessionById(sessionId) {
    return prismaClient.session.findFirstOrThrow({
      where: { id: sessionId },
    }).catch(createPrismaErrorParser({
  ...
  async refreshSession(userId, sessionId) {
    ...
    const expiresAt = Date.now() + serverConfig.service_configs.session.expire_time;
